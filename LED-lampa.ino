int voltPin = A2; //pin som läser av potentiometern
int readVal; // Variabel som håller läsvärdet från potentiometern
int i; //skrivvärdet för PWM

//Statemachine LED variabler
int state_prev_led = -1; //Startar i väntande läge
int state_led = -1;
long prev_v_i = 0;
long prev_r_i = 0;
long prev_g_i = 0;
...
bool write_g = false;
bool write_b = false;
int upphamtning_spel = 1; // större värden gör det enklare att "hämta upp" en färg i färgblandar-läge
...
//Statemachine favorite variabler
int state_prev_fav = 1; //Startar med favoritläge aktiverat
int state_fav = 1;
...
//Funktioner
// Läser av potentiometer och skalar ned värdet
void check_i() {
  readVal = analogRead(voltPin);
  i = map (readVal, 0, 1023, 0, 255);
}

// skriver ett värde till pwm-pin
void write_pwm(int pin_output, long pwm_value) {
  analogWrite (pin_output, pwm_value);
  if (pin_output == led_v) { //vit
    prev_v_i = pwm_value;
    write_v = false; //ser till att den inte skriver i-värde direkt om den hoppar till färgblandare
  }
...
void loop() {
  ...
  //Färgblandar-styrning
  if (state_s1 == 4) { // när switchen har värdet 4 har den uppfattat ett giltigt knapptryck
  ...
  //Favourites styrning. Se timer-funktion nedan. Potentiometers möjliga värden (0-255) indelade i bitar som motsvarar ett preset-läge. 
